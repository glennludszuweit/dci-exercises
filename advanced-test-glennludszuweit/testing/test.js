...
describe('convertObjectToArray should', () => {
  ...
  `(
    'convert an object containing properties into a nested array populated by the object properties',
    ({ input, expected }) => {
...
describe('convertArrayData should', () => {
  ...
  `(
    'switch first two array items and replace the third',
    ({ input, expected }) => {
...
describe('createGrid should', () => {
  test.each`
    ...
    ${2} | ${'f'} | ${[['f', 'f'], ['f', 'f']]}
  `(
    'create a grid which is $size by $size and populated with $char',
    ({ size, char, expected }) => {
...
describe('wordConverter should', () => {
  test.each`
    ...
    ${[1, 2, 3]}                    | ${'s'}   | ${['1s', '2s', '3s']}
  `(
    'return $expected when $suffix is added to each $word',
    ({ word, suffix, expected }) => {
...
describe('calculateHours should', () => {
  test.each`
    input                                                                                                                                                                                                    | expected
    ${[{ day: 'Monday', start: 8, end: 17 }, { day: 'Tuesday', start: 9, end: 15 }, { day: 'Wednesday', start: 10, end: 18 }, { day: 'Thursday', start: 7, end: 14 }, { day: 'Friday', start: 6, end: 12 }]} | ${36}
    ${[{ start: 2, end: 10 }, { start: 1, end: 7 }, { start: 90, end: 100 }, { start: 7, end: 14 }]}                                                                                                         | ${31}
...
describe('Course details should', () => {
  ...
  `(
    'return the description: $expected',
    ({ teacher, type, number, expected }) => {
      expect(new Course(teacher, type, number).details()).toStrictEqual(
        expected
      );
    }
  );
...
describe('Course spaceNeeded should', () => {
  ...
  `(
    'return the description: $expected',
    ({ teacher, type, number, expected }) => {
      expect(new Course(teacher, type, number).spaceNeeded()).toStrictEqual(
        expected
      );
    }
  );
